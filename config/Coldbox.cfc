component {
	...
	function configure(){
		...
		settings = {
			...
			dummySetting = "this is a dummy setting",
			dummySettingVersion = "2.13.25"
		...
		interceptors = [
			...
			// {
            //     class="interceptors.sampleInterceptor"
            // }
		...
		moduleSettings = {
			...
			cbauth : {				
				...
				userServiceClass : "models.Users.UserService"
			...
			cbsecurity : {
				...
				* your service of choice alongside the `BasicAuthValidator`
				...
				basicAuth : {
					...
					users : {
						"admin" : { password : "password", permissions : "write", roles : "admin" },
						"guest" : { password : "password", permissions : "write", roles : "user" }
					}
				...
				firewall : {
					...
					// Secure specific routes using firewall rules
					"rules" : [
						{
							// "whitelist" : false,  // This means the rule applies to these endpoints
							// "match"     : "api/private",  // URL pattern to protect
							// "methods"   : "GET,POST",  // HTTP methods this rule applies to
							// "roles"     	: "admin",  // Required role to access this route
							//"permissions"	: "write",  // (Optional) Permissions check
							// "action"    : "override",
							// "secureList" : "^api.*"
							"secureList"	: "api.private.index"
						}
					]
				...
				*/
				...
				// WireBox ID of the user service to use when leveraging user authentication, we default this to whatever is set
				// by cbauth or basic authentication. (Optional)
				"userService"     : "models.Users.UserService",
				// The name of the variable to use to store an authenticated user in prc scope on all incoming authenticated requests
				"prcUserVariable" : "oCurrentUser",
