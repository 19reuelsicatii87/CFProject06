...
component accessors="true" {
	...
	function setPermissions( permissions ){
		...
		// writeOutput(arguments.permissions & " isSimpleValue: " & isSimpleValue( arguments.permissions ));
		
		// writeOutput("<br>");
		// writeOutput(arguments.permissions & " isArray: " & isArray( arguments.permissions ));

		// writeOutput("<br>");
		// writeOutput(arguments.permissions & " isArray: " & isArray( listToArray(arguments.permissions)));

		// writeOutput("<br>");
		// writeOutput(arguments.permissions & " ----- " & serializeJSON(listToArray(arguments.permissions)));
	...
	struct function getJWTCustomClaims( required struct payload ){
		return { 
			"role" : variables.roles.toList(),
			"customField1": "Weng is Awesome",
			"customField2": "Weng is super Great!!"
		};
	}
